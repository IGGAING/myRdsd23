...
# SQL query to order the donations by MP and order desc 
donations_by_MP <- dbGetQuery(con, "
SELECT distinct m.id,m.name,sum(value) as total_money
FROM payments p
...
# Print the first row of the result that is going to be MP with most amount of money 
print(donations_by_MP[1, ])
```
...
# Retrieve the data from the 'payments_db' table
payments <- tbl(con, "payments") %>%
  collect()

# Retrieve the data from the 'members_db' table
members <- tbl(con, "members") %>%
  collect()

# Retrieve the data from the 'member_appgs' table
member_appgs <- tbl(con, "member_appgs") %>%
  collect()

# Retrieve the data from the 'parties' table
parties <- tbl(con, "parties") %>%
  collect()

# Retrieve the data from the 'party_donations' table
...
# Order the donations by MP and select the MP with the highest total amount of money
...
# Print the first row of the result, which represents the MP with the highest amount of money
print(donations_by_MP[1, ])
```
...
# SQL to get the different entites
Distinct_Entities <- dbGetQuery(con, "
...
# Retrieve the count of distinct entities that have made payments to MPs
...
# Print the count of distinct entities
cat("Number of distinct entities that donated to MPs:", entity_count, "\n")
```
...
# Calculate the total donations for each year
total_year_donations <- party_year_amounts %>%
  group_by(year) %>%
  summarise(total_donations = sum(total_year_donations))

# Calculate the proportion of each party's donations for each year
